"""
AEGIS-ADS v11.0 - THREE‑LAYER MANDATORY ARCHITECTURE
"""
...
# ============================================================
# Whitelist (لن يتم حظر هذه العناوين)
# ============================================================
WHITELIST_IPS = set()
...
# ============================================================
# وضع المراقبة (False = مراقبة فقط، True = حظر تلقائي)
# ============================================================
ENABLE_BLOCKING = False

def block_ip_30x(ip: str, reason: str = "Manual") -> dict:
    """حظر IP مع 30 قاعدة Windows Firewall، مع احترام القائمة البيضاء"""
...
# ============================================================
# Packet Feature Extraction for 10 numerical features
# ============================================================
_packet_time = {}
_packet_count = {}

def extract_10_features(packet_info: dict) -> np.ndarray:
    src = packet_info.get('src_ip', '0.0.0.0')
    now = time.time()

    proto = packet_info.get('transport', 'TCP')
    proto_map = {'TCP': 6, 'UDP': 17, 'ICMP': 1}
    protocol = proto_map.get(proto, 6)

    last = _packet_time.get(src, now)
    flow_duration = max(0.001, now - last)
    _packet_time[src] = now

    _packet_count[src] = _packet_count.get(src, 0) + 1
    total_fwd = _packet_count[src]

    total_bwd = 0

    length = packet_info.get('length', 64)
    fwd_len_max = length
    fwd_len_min = length

    flags = packet_info.get('flags', '')
    syn_count = 1 if (proto == 'TCP' and 'SYN' in str(flags)) else 0
    ack_count = 1 if (proto == 'TCP' and 'ACK' in str(flags)) else 0

    avg_size = length

    packets_per_sec = total_fwd / flow_duration if flow_duration > 0 else 1.0
    packets_per_sec = min(packets_per_sec, 1000)

    return np.array([protocol, flow_duration, total_fwd, total_bwd,
                     fwd_len_max, fwd_len_min, syn_count, ack_count,
                     avg_size, packets_per_sec], dtype=np.float32)

def build_packet_text(info: dict) -> str:
    text = f"Protocol: {info.get('transport', 'Unknown')} "
    text += f"Source IP: {info.get('src_ip', '0.0.0.0')} "
    text += f"Destination IP: {info.get('dst_ip', '0.0.0.0')} "
    text += f"Packet Length: {info.get('length', 0)} bytes. "
    if info.get('flags'):
        text += f"TCP Flags: {info.get('flags')}. "
    return text.strip()
...
def process_tshark_line(line):
    ...
    if not model_loader.is_ready:
        if packet_count == 1:
            logger.error("🚨 Models are NOT loaded! Please check core/models_loader.py and model files.")
        return
    ...
    try:
        ...
        if src in PROTECTED_IPS or src in WHITELIST_IPS:
            return
        ...
        info = {
            ...
            'dst_ip': dst,
            'length': length,
            'transport': 'ICMP' if length < 200 else 'TCP',
            'src_port': 0,
            'dst_port': 0,
            'flags': ''
        }
        ...
        try:
            features_10 = extract_10_features(info)
        except Exception as e:
            logger.error(f"Feature extraction error: {e}")
            return

        is_anomaly, anomaly_score = model_loader.predict_l1_anomaly(features_10)
        if not is_anomaly:
            if packet_count % 100 == 0:
                print(f"\r[Pkts:{packet_count} | Det:{len(detections)}]", end='', flush=True)
            return

        is_attack, l2_conf = model_loader.predict_l2_attack(features_10)
        if not is_attack:
            if packet_count % 100 == 0:
                print(f"\r[Pkts:{packet_count} | Det:{len(detections)}]", end='', flush=True)
            return

        packet_text = build_packet_text(info)
        deep_is_attack, deep_conf = model_loader.predict_l3_deep(packet_text)

        if deep_is_attack:
            action_text = 'Blocked'
            severity = 'CRITICAL' if deep_conf > 85 else 'HIGH'
            result = block_ip_30x(src, f'L3-Confirmed: {deep_conf:.1f}%')
            logger.warning(f"🔒 BLOCKED: {src} (L1:{anomaly_score:.1f}, L2:{l2_conf:.1f}, L3:{deep_conf:.1f}%)")
        else:
            action_text = 'Monitored (L3 Rejected)'
            severity = 'MEDIUM'
            result = None
            logger.warning(f"👁️ MONITORED: {src} (L1:{anomaly_score:.1f}, L2:{l2_conf:.1f}, L3:{deep_conf:.1f}%) – L3 did not confirm")

        alert = {
            'time': datetime.now().isoformat(),
            'attack': 'Confirmed Attack' if deep_is_attack else 'Suspicious',
            'src': src,
            'dst': dst,
            'anomaly_score': round(anomaly_score, 1),
            'l2_conf': round(l2_conf, 1),
            'l3_conf': round(deep_conf, 1),
            'confidence': round(deep_conf, 1),
            'severity': severity,
            'action': action_text,
            'rules_added': result.get('rules_added', 0) if result else 0
        }
...
def start_tshark_capture():
    ...
    # ⚠️ هام: عند تشغيل نقطة الوصول (Hotspot)، استخدم الواجهة 5 (الاتصال المحلي* 2)
    # إذا كنت متصلاً بالإنترنت مباشرة عبر Wi-Fi، استخدم الواجهة 4
    CAPTURE_INTERFACE = '5'   # غيّر إلى '4' إذا لم تستخدم Hotspot
    logger.info(f"🔌 Starting packet capture on interface {CAPTURE_INTERFACE}")

    def loop():
        cmd = [TSHARK_PATH, '-i', CAPTURE_INTERFACE, '-T', 'fields', '-e', 'ip.src', '-e', 'ip.dst', '-e', 'frame.len', '-l']
...
async def api_status():
    ...
    try:
        ...
        pass
    if not interfaces:
        interfaces = [{"name": "5", "ip": "192.168.137.1"}]
...
# Email Settings (اختصاراً، يمكنك إبقاؤها كما هي)
...
if __name__ == "__main__":
    print("=" * 60)
    print("  🛡️ AEGIS-ADS v11.0 - THREE‑LAYER MANDATORY ARCHITECTURE")
    print("=" * 60)
