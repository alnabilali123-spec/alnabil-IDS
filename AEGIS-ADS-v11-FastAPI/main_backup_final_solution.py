"""
AEGIS-ADS v11.0 - FINAL VERSION WITH WHITELIST
"""
import sys, os, time, threading, smtplib, asyncio, psutil, subprocess, json, re, shutil, numpy as np, numpy as np
...
alerts = []
logs_storage = []
# ping_tracker removed (now using AI models only)
...
def process_tshark_line(line):
    ...
    packet_count += 1
    # طباعة تصحيحية (يمكن إزالتها لاحقاً)
    # print(f"DEBUG: Packet received - count={packet_count}")
    try:
        ...
        now = time.time()
        # تجنب تحليل العناوين المحمية أو البيضاء
        if (length < 200 and src.startswith('192.168.137.') and src != '192.168.137.1'
                and src not in WHITELIST_IPS):
            # t = ping_tracker[src] removed
            t['count'] += 1
            # عتبة مرتفعة (50 ping في 5 ثوانٍ) لتجنب الحظر الخاطئ
            if t['count'] >= 50 and (now - t['start']) < 5:
...
if __name__ == "__main__":
    print("=" * 60)
    print("  🛡️ AEGIS-ADS v11.0 - FINAL VERSION WITH WHITELIST")
    print("=" * 60)
